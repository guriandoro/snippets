...
make_sandbox --add_prefix=rm_2_ /opt/percona_server/5.7.17  -- --sandbox_port=12441 --no_confirm

# no prompt for confirmation and no options used shown
make_sandbox --add_prefix=rm_3_ /opt/percona_server/5.7.17  -- --sandbox_port=12442 --no_show

# stops the server, if it was started after loading the grants
make_sandbox --add_prefix=rm_4_ /opt/percona_server/5.7.17  -- --sandbox_port=12443 --no_show --no_run

# add variables to be used on the my.sandbox.cnf file
make_sandbox --add_prefix=rm_5_ /opt/percona_server/5.7.17  -- --sandbox_port=12444 --no_show -c innodb_file_per_table=0 -c innodb_file_format=Antelope

# add many variables to be used on the my.sandbox.cnf file
make_sandbox --add_prefix=rm_6_ /opt/percona_server/5.7.17  -- --sandbox_port=12445 --no_show -c \
"
...
make_replication_sandbox --replication_directory=repl_rm_7_ --sandbox_base_port=12446 /opt/percona_server/5.7.17 

# with only one slave
make_replication_sandbox --replication_directory=repl_rm_8_ --sandbox_base_port=12449 --how_many_slaves=1 /opt/percona_server/5.7.17 

# master-master
# for some reason it starts nodes starting from sandbox_base_port+1 (bug?)
make_replication_sandbox --replication_directory=repl_rm_9_ --sandbox_base_port=12451 --master_master /opt/percona_server/5.7.17 

# 4-node ring
# for some reason it starts nodes starting from sandbox_base_port+1 (bug?)
make_replication_sandbox --replication_directory=repl_rm_10_ --sandbox_base_port=12454 --circular=4 /opt/percona_server/5.7.17 

# setting master options
make_replication_sandbox --replication_directory=repl_rm_11_ --sandbox_base_port=12459 --how_many_slaves=1 \
...
make_replication_sandbox --replication_directory=repl_rm_12_ --sandbox_base_port=12461 --how_many_slaves=1 \
--slave_options="-c report-host=127.0.0.1" \
/opt/percona_server/5.7.17 

# setting slave options. Important for tools like pt-table-checksum to work properly
make_replication_sandbox --replication_directory=repl_rm_13_ --sandbox_base_port=12463 --how_many_slaves=2 \
